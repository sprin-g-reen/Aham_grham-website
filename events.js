...
function createTextTexture(gl, text, font = 'bold 30px IBM Plex Sans', color = 'black') {
...
// --- CLASSES ---
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = 'bold 30px IBM Plex Sans' }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class CircularGalleryApp {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
  ...
  onCheck() {
    ...
    this.scroll.target = (this.scroll.target < 0 ? -1 : 1) * width * itemIndex;
  }
  onResize() {
...
document.addEventListener('DOMContentLoaded', () => {
  const items = [
    { image: 'assets/22.jpg', text: 'Primal Breath' },
    { image: 'assets/23.jpg', text: 'Lunar Gaze' },
    { image: 'assets/24.jpg', text: 'Desert Stillness' },
    { image: 'assets/25.jpg', text: 'Forest Flow' },
    { image: 'assets/26.jpg', text: 'Urban Reset' }
  ];

  const container = document.getElementById('circular-gallery');
  if (container) {
    new CircularGalleryApp(container, { 
      items, 
      bend: 3, 
      textColor: '#ffffff', 
      borderRadius: 0.05, 
  ...
  // Chroma Grid Logic
  const chromaContainer = document.querySelector('.chroma-grid-container');
  if (chromaContainer) {
    chromaContainer.addEventListener('mousemove', (e) => {
      const rect = chromaContainer.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      chromaContainer.style.setProperty('--x', `${x}px`);
      chromaContainer.style.setProperty('--y', `${y}px`);
      
      // Update individual cards for spot lighting effect
      const cards = chromaContainer.querySelectorAll('.chroma-card');
      cards.forEach(card => {
        const cardRect = card.getBoundingClientRect();
        const cardX = e.clientX - cardRect.left;
        const cardY = e.clientY - cardRect.top;
        card.style.setProperty('--mouse-x', `${(cardX / cardRect.width) * 100}%`);
        card.style.setProperty('--mouse-y', `${(cardY / cardRect.height) * 100}%`);
      });
    });
  }
});
